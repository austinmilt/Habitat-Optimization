...
sets
    ...
    Clusters 'possible clusters up to the number of targets' / 1*2 /;
...
positive variable yByp_By_yBypp 'yByp_By_yBypp  = yBypp / yByp'
...
* DEFINE EQUATIONS
eq_objective..
    objective =e= sum(C, weightPerformance*pError(C) - weightDistribution*dError(C)) + weightCluster*(nC/nT);

eq_performance(C)..
    pError(C) =e= sum(T, yBypp(C,T)) / sum(T, yByp(C,T));
